...
function PlayCommand(room, message, authorName, authorHash, reply) {
  ...
  let splitMessage = message.split(" "); //메시지를 띄어쓰기 단위로 나눠요.
  ...
  if (commandSW(`음악`)) {
    ...
    if (splitMessage[2] === "검색") { //음악을 검색해요.
      ...
      reply(UserManager.getMusicSearch(authorHash, musicName)[0]); //img
      reply(UserManager.getMusicSearch(authorHash, musicName)[1]); //lyrics
    }
  ...
  if (commandSW(`주식`)) {
    ...
    switch (type) {
      case "현황": //주식 현황을 출력해요.
        reply(Object.entries(UserManager.getStockList()).map(([key, value]) => `${key} : ${value}`).join("\n"));
        break;
      case "구매": //주식을 구매해요.
