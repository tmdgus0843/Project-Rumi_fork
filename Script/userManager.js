    ...
    return {
      ...
      getMusicSearch: function (id, song) {
        ...
        let musicLyrics = org.jsoup.Jsoup.connect(`https://www.melon.com/song/detail.htm?songId=${music["SONGID"]}`)
          .ignoreHttpErrors(true)
          .get();
        let lyrics = musicLyrics.select("div#d_video_summary")
          .html()
          .split("--> ")[1]
          .replace(/<br>/g, "");
        return [
          ...
          [
            `${musicLyrics.select("div.song_name").text().replace("곡명", "")}의 가사에요.`,
            Library.More,
            `${lyrics}`
          ]
        ];
        ...
        // image : music["ALBUMIMG"].replace("resize/40/quality/80/optimize", "")
        // songid : music["SONGID"]
      },
